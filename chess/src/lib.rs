use dartfrog_lib::{get_server_address, handle_plugin_update, send_to_frontend, update_subscriber_clients, PluginClientState, PluginMessage, PluginMetadata, PluginServiceState, PluginState};
...
fn init(our: Address) {
    ...
    let try_ui = kinode_process_lib::http::secure_serve_ui(&our, "chess-ui", vec!["/"]);
    match try_ui {
        Ok(()) => {}
        Err(e) => {
